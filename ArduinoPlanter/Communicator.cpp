...
input_result Communicator::handleJson(char const * buffer)
{
	...
	{
		if (!strcmp(configuration_setting_names[CONF_ACIDITY_PIN], it->key))
		{
			configuration->acidity_pin = (analog_in_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_AIR_READ_PIN], it->key))
		{
			configuration->air_read_pin = (digital_in_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_WATER_BOTTOM_PIN], it->key))
		{
			configuration->water_bottom_pin = (digital_in_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_WATER_TOP_PIN], it->key))
		{
			configuration->water_top_pin = (digital_in_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_LAMP_PIN], it->key))
		{
			configuration->lamp_pin = (digital_out_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_PUMP_PIN], it->key))
		{
			configuration->pump_pin = (digital_out_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_LAMP_CYCLE_TIME], it->key))
		{
			configuration->lamp_cycle_time = (milliseconds_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_LAMP_ACTIVE_TIME], it->key))
		{
			configuration->lamp_active_time = (milliseconds_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_PUMP_CYCLE_TIME], it->key))
		{
			configuration->pump_cycle_time = (milliseconds_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_PUMP_ACTIVE_TIME], it->key))
		{
			configuration->pump_active_time = (milliseconds_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_AIR_READ_INTERVAL], it->key))
		{
			configuration->air_read_interval = (milliseconds_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_REPORT_INTERVAL], it->key))
		{
			configuration->report_interval = (milliseconds_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_REMOTE_CONTROL_TIMEOUT], it->key))
		{
			configuration->remote_control_timeout = (milliseconds_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_STARTUP_DELAY], it->key))
		{
			configuration->startup_delay = (milliseconds_t)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_SERIAL_PORT_SPEED], it->key))
		{
			Serial.println("Serial port may not be reconfigured.");
			result = REQUEST_INVALID;
		}
		else if (!strcmp(configuration_setting_names[CONF_WATER_SENSOR_VALUE_WHEN_WET], it->key))
		{
			configuration->water_sensor_value_when_wet = (int)it->value;
			result = REQUEST_RECONFIGURE;
		}
		else if (!strcmp(configuration_setting_names[CONF_AIR_SENSOR_TYPE], it->key))
		{
